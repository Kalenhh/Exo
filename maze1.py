...
def generate_maze(width=30,eight=30) :
	...
	cluster_dict = { cell_dict[o].cluster : [cell_dict[o]]  for o in cell_dict}
	...
	while len(cell_ind)>1 :
		...
		# CHANGER LES CLUSTER
		for i in cluster_dict[cluster_to_change] :

			i.cluster = cluster_to_stay


		cluster_dict[cluster_to_stay] += cluster_dict[cluster_to_change]
		del cluster_dict[cluster_to_change]

	

		for i in cluster_dict[cluster_to_stay] :
			curr = i
		...
		for i in cluster_dict[cluster_to_stay] :
			curr = i					# ENLEVER LES CELLULES TERMINEES
...
b = tk.Button(app,text="Generer",command=generate_maze)
b.pack(side="right")
